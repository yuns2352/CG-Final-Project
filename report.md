...
> 컴퓨터 그래픽스 최종 과제
> 게임 링크: `https://yuns2352.github.io/CG-Final-Project/`
...
## 7. 빌드 / 실행
- 웹: `https://yuns2352.github.io/CG-Final-Project/`
...
### 배운 점
- **DDGI의 본질과 구현을 분리해서 보게 되었다.** 처음엔 "DDGI = 레이트레이싱 + octahedral atlas"라는 형태에 집착했지만,
  구현을 진행하면서 DDGI를 정의하는 것은 *형태*가 아니라 **irradiance probe field·표면의 probe 샘플링·동적 갱신**이라는
  *원리*임을 이해했다. 그 원리를 지키면서 입사광 수집만 큐브맵 래스터화로 치환할 수 있었다.
- **WebGPU의 제약이 곧 설계 결정으로 이어진다는 것**을 체감했다. 셰이더에서 다수의 큐브맵을 동적 인덱싱할 수 없다는
  한 가지 제약이, "픽셀 단위 블렌딩"을 포기하고 "타일 단위 probe 선택"을 택하게 만든 핵심 이유였다.
- **실시간 성능은 기능이 아니라 예산 관리**라는 점. probe를 다 갱신하지 않고 amortize하고, 워밍업을 로딩 뒤로 옮기는 등
  "언제·얼마나 계산할지"를 조절하는 것이 화질만큼 중요했다.

### 아쉬운 점 / 한계
- **타일 경계의 불연속**: probe 선택이 타일 단위라 면이 바뀌는 경계에서 간접광이 매끄럽지 않다. octahedral atlas로
  픽셀 단위 보간을 했다면 사라졌을 부분이다.
- **가시성(light leak) 처리를 본선에서 제외**했다. line-of-sight 방식을 시험했으나 단일 동굴에서는 효과가 미미해,
  안정성을 택해 빼는 결정을 내렸다. 더 복잡한 레벨이었다면 반드시 필요한 기능이었을 것이다.
- **probe 해상도·개수를 성능 때문에 보수적으로** 잡아, 더 정교한 간접광은 양보했다.

### 다음에 해보고 싶은 것
- **Octahedral atlas 기반 DDGI**로 발전시켜 픽셀 단위 8-probe 트라이리니어 블렌딩과 Chebyshev 가시성을 구현하기.
- **WebGPU compute 셰이더**로 큐브맵 게더를 SDF 레이마칭으로 교체해 정통 트레이스드 DDGI에 근접시키기.
- GI를 더 깊게 메커닉화하기

### 한 줄 소회
- **화려한 완성본보다, 한계를 알고 무엇을 단순화할지 스스로 판단하는 과정에서 더 많이 배웠다.
